...
export default function SalesDashboard() {
    ...
    const [selectedTableDetails, setSelectedTableDetails] = useState(null); // { label, orders }
    ...
    useEffect(() => {
        ...
        fetchSales();
        const interval = setInterval(fetchSales, 30000); // Auto-refresh every 30s
                ...
                <header style={{
                    ...
                    <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem' }}>
                            ...
                            <h1 className="text-display" style={{ color: 'var(--text-primary)' }}>Sales Dashboard</h1>
                        </div>
                        <p className="text-secondary" style={{ fontWeight: 500, fontSize: 'var(--text-body)' }}>Comprehensive business intelligence and performance tracking</p>
                    </div>
                                ...
                                {[
                                    ...
                                    <div key={i} style={{
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.75rem' }}>
                                            <h3 className="text-display" style={{ fontSize: 'var(--text-h1)', color: 'var(--text-primary)', margin: 0, fontVariantNumeric: 'tabular-nums' }}>{stat.value}</h3>
                                            {stat.trend !== undefined && <TrendIndicator value={stat.trend} />}
                                        </div>
                                        <p className="text-secondary" style={{ fontSize: 'var(--text-xs)', margin: 0 }}>{stat.subtitle || 'vs. previous day'}</p>
                                    </div>
                                ))}
                            </div>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '2rem' }}>
                                        ...
                                        <div style={{ background: 'white', padding: '2rem', borderRadius: '20px', borderWidth: '1px', borderStyle: 'solid', borderColor: 'var(--border)', boxShadow: 'var(--shadow-sm)' }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                                                <h2 className="text-heading" style={{ color: 'var(--text-primary)' }}>Hourly Traffic Density</h2>
                                                <span className="text-caption" style={{ color: 'var(--text-muted)' }}>Real-time Distribution</span>
                                            </div>
                                        ...
                                        {/* Category Engagement */}
                                        <div style={{ background: 'white', padding: '2rem', borderRadius: '20px', borderWidth: '1px', borderStyle: 'solid', borderColor: 'var(--border)', boxShadow: 'var(--shadow-sm)' }}>
                                            <h2 className="text-heading" style={{ color: 'var(--text-primary)', marginBottom: '2rem' }}>Menu Engagement</h2>
                                        ...
                                        <div style={{ background: 'white', padding: '2rem', borderRadius: '20px', borderWidth: '1px', borderStyle: 'solid', borderColor: 'var(--border)', boxShadow: 'var(--shadow-sm)' }}>
                                            <h2 style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--text-primary)', marginBottom: '1.5rem' }}>Most Popular Tables</h2>
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                                {(data.summary.topTables || []).length > 0 ? (
                                                    data.summary.topTables.map((table, idx) => (
                                                        <div key={idx}
                                                            onClick={() => {
                                                                const tableOrders = data.orders.filter(o => o.table === table.label);
                                                                setSelectedTableDetails({ label: table.label, orders: tableOrders });
                                                            }}
                                                            style={{
                                                                display: 'flex',
                                                                alignItems: 'center',
                                                        ...
                                                        >
                                                            ...
                                                            <div style={{ flex: 1 }}>
                                                                ...
                                                                <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', margin: 0 }}>
                                                                    {table.sessionCount} Customer{table.sessionCount !== 1 ? 's' : ''} Today
                                                                </p>
                                                            </div>
                                                            <div style={{ textAlign: 'right' }}>
                                                                <p style={{ fontWeight: 800, color: 'var(--primary)', margin: 0 }}>{formatCurrency(table.revenue)}</p>
                                                                <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)' }}>Revenue Today</span>
                                                            </div>
                                                        </div>
                                                    ))
                                                ) : (
                                                    <p style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.875rem', padding: '2rem' }}>Awaiting initial table data...</p>
                                                )}
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                                <thead>
                                                    <tr style={{ textAlign: 'left', background: '#f8fafc', borderBottom: '2px solid #f1f5f9' }}>
                                                        <th style={{ padding: '1.25rem 2rem' }} className="text-caption">Menu Item</th>
                                                        <th style={{ padding: '1.25rem' }} className="text-caption">Classification</th>
                                                        <th style={{ padding: '1.25rem', textAlign: 'center' }} className="text-caption">Units</th>
                                                        <th style={{ padding: '1.25rem 2rem', textAlign: 'right' }} className="text-caption">Financial Impact</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                ...
                selectedTableDetails && (
                    <div style={{
                        ...
                        <div style={{
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <div>
                                    <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--text-primary)', margin: 0 }}>Table {selectedTableDetails.label} Summary</h2>
                                    <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)', margin: 0 }}>Activity log for {selectedDate}</p>
                                </div>
                                <button
                                    onClick={() => setSelectedTableDetails(null)}
                                    style={{
                                        width: '36px', height: '36px', borderRadius: '50%', background: 'var(--surface-muted)', border: 'none', cursor: 'pointer', fontSize: '1.25rem', color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', justifyContent: 'center'
                                    }}
                                >✕</button>
                            </div>

                            <div className="scroll-hide" style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '1.5rem', paddingRight: '0.5rem' }}>
                                {selectedTableDetails.orders.length > 0 ? (
                                    selectedTableDetails.orders.map((order, idx) => (
                                        <div key={idx} style={{
                                            border: '1px solid var(--border)',
                                            borderRadius: '16px',
                                            padding: '1.25rem'
                                            ...
                                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
                                                <div>
                                                    <p style={{ fontWeight: 800, fontSize: '0.875rem', color: 'var(--primary)', margin: 0 }}>Order ORD-{order.shortId}</p>
                                                    <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', margin: 0 }}>{formatTime(order.time)}</p>
                                                </div>
                                                <p style={{ fontWeight: 800, color: 'var(--text-primary)', fontSize: '1rem', margin: 0 }}>{formatCurrency(order.total)}</p>
                                            </div>
                                    ...
                                    <div style={{ textAlign: 'center', padding: '3rem 0', opacity: 0.5 }}>
                                        ...
                                        <p>No orders recorded for this table today.</p>
                                    </div>
                                )}
                            </div>

                            <div style={{ borderTop: '1px solid var(--border)', paddingTop: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <span style={{ fontWeight: 700, color: 'var(--text-secondary)' }}>Total Daily Revenue</span>
                                <span style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--primary)' }}>
                                    {formatCurrency(selectedTableDetails.orders.reduce((sum, o) => sum + o.total, 0))}
                                </span>
                            </div>
                        </div>
                    </div>
